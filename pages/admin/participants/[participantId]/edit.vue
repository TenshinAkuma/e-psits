<template>
   ...
   <section v-else>
      ...
      <ParticipantsTabs activeTab="details"/>
		...
		<br />
		<div class="row">
			<form :id="`edit-participant-form-${participantId}`" class="col-8"
			@submit.prevent="OnSaveEdit">
				<dl class="row gy-2">
					...
					<dd class="col-sm-8 d-flex gap-2">
						<select class="form-select border-secondary" v-model="ParticipantEdit.sex">
					...
					<dd class="col-sm-8">
						...
						v-model="ParticipantEdit.email" required>
					</dd>
					...
					<dd class="col-sm-8">
						<select class="form-select border-secondary" v-model="ParticipantEdit.institution_id" required>
							<option v-for="institution in institutionOptions"
							:key="institution.id" :value="institution.id">{{ institution.name }}</option>
						</select>
					</dd>
...
<script setup>
	...
	const LoadData = async () => {
		...
		const { data: institutions, error: institutionsError } = await $fetch(`/api/institutions/options`, {
			method: "GET"
		})
