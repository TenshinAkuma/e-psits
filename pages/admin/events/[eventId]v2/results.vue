				...
				<table class="table table-bordered align-middle">
					...
					<tbody>
						<tr
							...
							<td v-for="criterion in EventCriteria"
							:key="criterion.id"
							class="text-center fs-7"
							:class="{'text-secondary': result.scores[criterion.id] == null}">
							{{`${CatchPoints(result.scores[criterion.id])} pts`}}
							</td>
...
<script setup>
	...
	const TransformEventResults = (results) => {
		...
		results.forEach((item) => {
			...
			if (participant.scores[criteria_id]?.score !== score) {
				participant.scores[criteria_id] = { score, points };
	...
	const CatchPoints = (scoreData) => {
		if (scoreData == null) {
			return 0;
		}
		return scoreData.points;
	}
</script>
