<template>
	<div class="fs-7">
		...
		<p v-if="!isEditing" class="fw-bold lh-sm">
			{{ new Date(participant.dob).toDateString() || "No data available." }}
		</p>
...
<script setup>
	...
	const participantEdit = ref({
		dob: new Date(participant.value?.dob).toISOString().split("T")[0],
	});
	...
	const OnSaveParticipantEdit = async () => {
		try {
			...
			if (_participantData.value?.error) {
				throw new Error("Error while updating participant birth date.")
			}
