<template>
	<div>
		...
		<form v-else @submit.prevent="OnSaveNewDob" class="mt-1">
			<input
				type="date"
...
<script setup>
	...
	const ParticipantDob = defineModel("ParticipantDob");

	const participant = ref({
		dob: new Date(ParticipantDob.value).toISOString().split("T")[0],
	});
	...
	const OnSaveNewDob = async () => {
		try {
			await execute();
			if (status.value == "success") {
				ToggleEdit();
				ParticipantDob.value = participant.value.dob;
			}
		} catch {
			console.log(error);
		}
