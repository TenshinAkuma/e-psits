...
<script setup>
	...
	const LoadData = async () => {
		...
		const { data: _participants, error: _participantsError } =
			await $fetch(`/api/event-registrations/${eventData.value?.id}`);
	...
	const payload = computed(() => {
		...
		return {
			...
			scores: Object.entries(result.value?.scores).map(
				([criteria_id, score]) => ({
	...
	const OnSaveEvaluation = async () => {
		...
		const { error } = await $fetch(`/api/event-results`, {
			method: "POST",
			body: payload.value,
		});
